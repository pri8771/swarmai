...
def test_put_approval_twice_rejected_and_row_unchanged(factory) -> None:
    ...
    grant = gw.make_approval(env, context=_context(), max_effect_count=1)
...
async def test_put_cannot_reset_used_count(factory) -> None:
    ...
    assert (await gw.execute_envelope(env, context=_context())).outcome == "succeeded"
...
async def test_revocation_is_monotonic_and_blocks_execution(factory) -> None:
    ...
    env = bind_lease(factory, env)
    grant = gw.make_approval(env, context=_context(), max_effect_count=3)
    env.approval_id = grant.approval_id
    assert gw.revoke_approval(
        grant.approval_id, context=_context(actor="operator"), reason="changed mind"
    )
    ...
    assert gw.revoke_approval(grant.approval_id, context=_context(actor="x"), reason="y") is False
...
def test_revoke_wrong_project_returns_false_and_leaves_grant_active(factory) -> None:
    ...
    env = bind_lease(factory, env)
    grant = gw_a.make_approval(env, context=_context())
...
async def test_project_b_cannot_fetch_project_a_approval(factory) -> None:
    ...
    with pytest.raises(ApprovalInvalidError, match="unknown_approval"):
        await gw_b.execute_envelope(env_b, context=_context("proj_b"))

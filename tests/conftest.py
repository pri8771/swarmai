...
def _allow_paid_enabled() -> bool:
    raw = (os.environ.get("SWARM_ALLOW_PAID") or "").strip().lower()
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    Fail closed: live_local refuses to run when SWARM_ALLOW_PAID is enabled.
    """
...
def live_fixture_url(live_fixture_credentials: dict[str, str]) -> Iterator[str]:
    ...
    if _allow_paid_enabled():
        pytest.fail("live_local fail-closed: SWARM_ALLOW_PAID must be false/unset")
    env = {
        **os.environ,

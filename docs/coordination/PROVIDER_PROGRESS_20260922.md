...
| Local runtimes | Ollama | Installed local route; actual historical brokered calls to gemma3:4b and qwen3.5:4b recorded in f393f7f, on execution tip 84df040b | Fresh run/current-source evidence remains separate from those September 21 local receipts. The older local-admission-reconcile script substitutes an adapter stub and is not live inference evidence |
...
## Owner selection and newly verified prerequisites

Owner response: "Yup lets reuse." Reuse Groq and OpenRouter remote accounts plus Ollama local runtime. No account signup/upgrade or broad provider rollout is needed. This selection follows a proposal to prepare bounds and then obtain live-call approval; it is not a new inference grant.

Code inspection identified an actual missing output-token bound in core adapters; CODEX_PROVIDER_OUTPUT_BOUND_20260922.md releases its isolated offline repair. Do not promise an executable remote qualification command yet: existing onboarding canary rejects remote providers, explicit OpenRouter routing/price restrictions are absent, core HTTP is synchronous inside the async adapter, and receipts do not retain rate-limit headers/provider cost. These are engineering prerequisites, not missing owner account names.

Evidence correction: docs/evidence/inf-121/local-admission-reconcile.json was generated by scripts/g12_local_admission_reconcile_proof.py with execute_one replaced by a stub. It proves simulated accounting only. Real historical local evidence is f393f7f:docs/evidence/g12/a5-current-tip-local-proof.json, tied to source 84df040b6ac5981856ab7f32b66966cc0cf38894, recorded September 21. Neither artifact qualifies the current remote pair.

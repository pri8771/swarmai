...
port = 56421
db = f'swarm_r28c_compat_{os.getpid()}_{int(time.time())}'
log_path = Path('/tmp/swarm-r28c-compatibility.log')
targets = [
    'tests/tools/test_v17_gateway_*.py',
    'tests/tools/test_adapter_registry.py',
    'tests/integration/db/test_action_receipts_durable.py',
    'tests/integration/db/test_approval_integrity.py',
    'tests/integration/db/test_effect_crash_window.py',
    'tests/integration/db/test_effect_transactions.py',
]
pytest_command = (
...
with log_path.open('w', encoding='utf-8') as log:
    ...
    try:
        ...
        # Expand the requested wildcard exactly as the shell would.
        expanded = sorted(str(p.relative_to(repo)) for p in (repo / 'tests/tools').glob('test_v17_gateway_*.py'))
        command = [str(venv / 'pytest'), '-q', *expanded, *targets[1:]]

...
repo=Path('/tmp/swarm-astra-r30b-prerequisite-20260922'); out=Path('/tmp/swarm-astra-r30b-p0-resume-20260922')
...
try:
    ...
    (out/'pg-focused-first.log').write_text(p.stdout); pg['exit']=p.returncode; print(p.stdout[-5000:])
finally:
    if pg['created']:
        with psycopg.connect(dbname='postgres',host=socket,port=port,autocommit=True) as c:
    ...
    (out/'pg-focused-first.json').write_text(json.dumps(pg,indent=2)); print(json.dumps(pg))

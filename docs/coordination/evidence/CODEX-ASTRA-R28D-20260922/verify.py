...
out = Path('/tmp/swarm-astra-review-20260922')
env = os.environ.copy()
...
run('focused-offline',['uv','run','--no-sync','pytest','tests/mission/test_mission_runtime_async_worker_bridge.py','tests/mission/test_no_auto_promote.py','-q','-rs','--junitxml=/tmp/swarm-astra-review-20260922/focused.xml'])
...
try:
    ...
    run('full-postgres',['uv','run','--no-sync','pytest','-q','-rs','--tb=short','--junitxml=/tmp/swarm-astra-review-20260922/full-postgres.xml'],pg_env)

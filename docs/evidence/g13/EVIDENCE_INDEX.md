...
|----------|-------|-------------|
| `g13/TASK_POOL_FREEZE_V2.md` | ART-V13-TASK-POOL (EXT-WORKER-PC-V2B-001-R2, provenance and independence corrected by -R3): `g13-pool-freeze-v2` — 240 input-only held-out **records**, 15 per required cell, sealed grader-reference interface, fail-closed digest/template independence checker, ten pinned identities, computed readiness | Artifact accept; authorisation to start counted qualification (`counted_qualification_ready=false`); that the 15 records per cell are 15 **independent** observations (measured: **5 archetypes per cell**, blocker **B7**); that the v2 verifier/generator/tests were ever executed; that the Actions `35587202715` offline pytest failure was reproduced or fixed (blocker **B8**) |

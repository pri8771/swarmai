...
Then: `SW-MERGE-W4` → SW-X2-S1 → `SW-MERGE-X` → Codex review of the integration range (B-03) → the owner merges integration into `dev` (C1) → later `dev`→`main` merge and tag (B-07, owner only).
...
Each SW check is a read-only `gh api` or `git` call. A missing sync point never makes a session wait: its STOP (S7) records the gap and pushes.

| SP | inference_server event (on `cursor/is-v23-integration-460c`) | swarmai session and step that consume it | SW check | If not reached |
|---|---|---|---|---|
| SP1 contract frozen | IS-W1-S10 merged by IS-W2-MERGE: `docs/api/v1/consumers/swarmai.md` | **SW-X1-S1** Step 0 (gate) | `gh api repos/pri8771/inference_server/contents/docs/api/v1/consumers/swarmai.md?ref=cursor/is-v23-integration-460c` returns 200 | X1 STOPs (S7) before any edit; W4-S1 records the adapter `pending (gate SP1)` |
| SP2 mock testable | same merge: `scripts/mock_splitsignal.py` + fixtures | **SW-X1-S1** Step 8 (optional run on port 8089) | `.../contents/scripts/mock_splitsignal.py?ref=...` returns 200 | Step 8 skipped and noted; the in-repo fake still covers the contract |
| SP3 key issuable | IS-W1-S11 merged and IS-W2-DEPLOY imported the SwarmAI key (`docs/evidence/m1/hosted-v05.md`) | no session: the owner's preflight stores `SPLITSIGNAL_API_KEY` (SEC-13) | SW-X2-S1 Step 0 `test -n "$SPLITSIGNAL_API_KEY"` | X2 STOPs (S6) |
| SP4 live non-streaming | IS-W2-DEPLOY smoke passed | **SW-X2-S1** Step 3, text call (`GET /v1/models` 200 + one chat) | smoke exit 0, or `calls[0].mode == "text"` with a receipt | exit 1/3 is recorded as `SP4: not reached (<status>)`; no retry |
| SP5 live streaming | IS-W4-MERGE redeploy, stream ends with `[DONE]` | **SW-X2-S1** Step 3, `--stream` call | `calls[1].mode == "stream"` without `partial_stream` | recorded as `SP5: not reached`; SP4 evidence is kept |
| SP6 joint finish | IS-W8-MERGE, `docs/evidence/v2.3-offline.md` cites `swarmai-consumer 1.x` | **SW-X2-S1** Step 4 + **SW-W4-S1** EXIT_CHECKLIST row "SplitSignal consumer adapter" | `gh api .../docs/evidence/v2.3-offline.md` contains `swarmai-consumer 1.` | recorded `SP6: pending`; not a STOP |

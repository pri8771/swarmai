# SW-FIX-RETRY — RetryOwner gives up when upstream Retry-After exceeds the cap (SplitSignal rule)
...
| Your branch | `cursor/sw-fix-retry-after` |
...
| Wave | FIX |
| Depends on | SW-W4-S1 |
| Handoff file | `docs/v2.3/sessions/SW-FIX-RETRY.md` |
...
```bash
git checkout -b cursor/sw-fix-retry-after origin/cursor/sw-v23-integration-460c
...
```bash
git fetch origin cursor/sw-v23-integration-460c
git cat-file -e origin/cursor/sw-v23-integration-460c:src/swarm/broker/retry.py && echo "OK src/swarm/broker/retry.py" || echo "MISSING src/swarm/broker/retry.py"
```
...
- `tests/broker/test_retry_after_cap.py` — modify
- `CHANGELOG.md` — modify (the F-06 line only)
- `docs/v2.3/sessions/SW-FIX-RETRY.md` — create (your handoff)
...
sudo -u postgres psql -c "CREATE DATABASE swarm_sw_fix_retry OWNER swarm;" || true
export SWARM_DATABASE_URL=postgresql+psycopg://swarm:swarm@127.0.0.1:5432/swarm_sw_fix_retry
uv run ruff check .
uv run mypy src/swarm
uv run alembic heads               # must print exactly ONE line ending in (head)
uv run pytest tests/broker tests/providers -q
...
git add src/swarm/broker/retry.py tests/broker/test_retry_after_cap.py CHANGELOG.md docs/v2.3/sessions/SW-FIX-RETRY.md
git status --porcelain            # nothing unexpected staged or left over
git commit -m "fix(broker): give up when upstream Retry-After exceeds max_retry_after_seconds (SW-FIX-RETRY)" -m "Session: SW-FIX-RETRY. Plan: docs/plans/v2.3/PLAN.md."
git push -u origin cursor/sw-fix-retry-after
gh pr create --draft --base cursor/sw-v23-integration-460c --head cursor/sw-fix-retry-after --title "[SW-FIX-RETRY] RetryOwner gives up when upstream Retry-After exceeds the cap (SplitSignal rule)" --body-file docs/v2.3/sessions/SW-FIX-RETRY.md
git ls-remote origin refs/heads/cursor/sw-fix-retry-after   # must print the same SHA as: git rev-parse HEAD
```
...
Create `docs/v2.3/sessions/SW-FIX-RETRY.md` with exactly these headings:
```markdown
...
1. Commit whatever is complete inside your own files: `git add <your files> docs/v2.3/sessions/SW-FIX-RETRY.md` then `git commit -m "WIP(SW-FIX-RETRY): <one-line reason>"`.
...
3. Push: `git push -u origin cursor/sw-fix-retry-after` (plus your suffix).
4. Open the draft PR against `cursor/sw-v23-integration-460c` with the title prefix `[BLOCKED]`, or record the compare URL `https://github.com/pri8771/swarmai/compare/cursor/sw-v23-integration-460c...cursor/sw-fix-retry-after?expand=1` in the handoff.
...
```markdown
### Codex review packet — SW-FIX-RETRY
...
- Files to review: `src/swarm/broker/retry.py`, `tests/broker/test_retry_after_cap.py`, `CHANGELOG.md`, `docs/v2.3/sessions/SW-FIX-RETRY.md`
- Review focus (AGENTS.md Code Review Rules): cross-tenant/project access; secret disclosure; financial accounting (unknown usage or cost is never zero); unbounded retries; silently changed model behaviour; recovery gaps after a crash/restart; unsupported release claims ("accepted", "complete"). Session-specific: over-cap or non-finite Retry-After is a terminal give-up (`retry_after_exceeds_cap`), never a retry at the cap; terminal classes and max_attempts keep precedence.

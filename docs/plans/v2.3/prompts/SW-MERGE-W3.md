# SW-MERGE-W3 — merge wave W3 session PRs into `cursor/sw-v23-integration-460c`
...
| Work branch | `cursor/v23-merge-w3` (local only; append a forced suffix once if your environment requires one) |
| Handoff | `docs/v2.3/sessions/SW-MERGE-W3.md` (committed into `cursor/sw-v23-integration-460c` together with the merges) |
...
|---|---|---|---|
| 1 | `SW-W3-S1` | `cursor/v23-w3-s1-api-routes` | required |
| 2 | `SW-W3-S2` | `cursor/v20-w3-s2-durable-wiring` | required |
| 3 | `SW-W3-S3` | `cursor/v23-w3-s3-cli` | required |
| 4 | `SW-W3-S4` | `cursor/v23-w3-s4-acceptance-probes` | required |
| 5 | `SW-W3-S5` | `cursor/v20-w3-s5-compose-smoke` | optional |

## 2. Setup
```bash
git status --porcelain            # must print nothing; otherwise STOP (M1)
git fetch origin
git checkout -B cursor/v23-merge-w3 origin/cursor/sw-v23-integration-460c
...
sudo -u postgres psql -c "CREATE DATABASE swarm_sw_merge_w3 OWNER swarm;" || true
export SWARM_DATABASE_URL=postgresql+psycopg://swarm:swarm@127.0.0.1:5432/swarm_sw_merge_w3
...
## 5. Handoff and push
Write `docs/v2.3/sessions/SW-MERGE-W3.md` with: BASE, the new head, one line per session (merged / already merged / not pushed / not ready / conflict / fails verify), every section 4 result line, and the section 8 packet. Then:
```bash
git add docs/v2.3/sessions/SW-MERGE-W3.md && git commit -m "docs(v2.3): SW-MERGE-W3 handoff"
...
```markdown
### Codex review packet — SW-MERGE-W3

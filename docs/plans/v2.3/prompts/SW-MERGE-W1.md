# SW-MERGE-W1 — merge wave W1 session PRs into `cursor/sw-v23-integration-460c`
...
| Work branch | `cursor/v23-merge-w1` (local only; append a forced suffix once if your environment requires one) |
| Handoff | `docs/v2.3/sessions/SW-MERGE-W1.md` (committed into `cursor/sw-v23-integration-460c` together with the merges) |
...
|---|---|---|---|
| 1 | `SW-W1-S1` | `cursor/v23-w1-s1-wdrr` | required |
| 2 | `SW-W1-S2` | `cursor/v23-w1-s2-sql-store` | required |
| 3 | `SW-W1-S3` | `cursor/v23-w1-s3-dispatch-intent` | required |
| 4 | `SW-W1-S4` | `cursor/v23-w1-s4-scheduler-epoch` | required |
| 5 | `SW-W1-S5` | `cursor/v23-w1-s5-pack-lifecycle` | required |
| 6 | `SW-W1-S6` | `cursor/v23-w1-s6-portability-v2` | required |
| 7 | `SW-W1-S7` | `cursor/v23-w1-s7-fleet-policy` | required |
| 8 | `SW-W1-S8` | `cursor/v23-w1-s8-ops-trace` | required |
| 9 | `SW-W1-S9` | `cursor/v20-w1-s9-pursuit-writethrough` | required |
| 10 | `SW-W1-S10` | `cursor/v20-w1-s10-durable-holds` | required |
| 11 | `SW-W1-S11` | `cursor/v23-w1-s11-router-client` | required |
| 12 | `SW-W1-S12` | `cursor/v20-w1-s12-console-ops` | required |
| 13 | `SW-W1-S13` | `cursor/v20-w1-s13-sandbox-killbound` | optional |

## 2. Setup
```bash
git status --porcelain            # must print nothing; otherwise STOP (M1)
git fetch origin
git checkout -B cursor/v23-merge-w1 origin/cursor/sw-v23-integration-460c
...
sudo -u postgres psql -c "CREATE DATABASE swarm_sw_merge_w1 OWNER swarm;" || true
export SWARM_DATABASE_URL=postgresql+psycopg://swarm:swarm@127.0.0.1:5432/swarm_sw_merge_w1
...
## 5. Handoff and push
Write `docs/v2.3/sessions/SW-MERGE-W1.md` with: BASE, the new head, one line per session (merged / already merged / not pushed / not ready / conflict / fails verify), every section 4 result line, and the section 8 packet. Then:
```bash
git add docs/v2.3/sessions/SW-MERGE-W1.md && git commit -m "docs(v2.3): SW-MERGE-W1 handoff"
...
```markdown
### Codex review packet — SW-MERGE-W1

# SW-MERGE-W2 — merge wave W2 session PRs into `cursor/sw-v23-integration-460c`
...
| Work branch | `cursor/v23-merge-w2` (local only; append a forced suffix once if your environment requires one) |
| Handoff | `docs/v2.3/sessions/SW-MERGE-W2.md` (committed into `cursor/sw-v23-integration-460c` together with the merges) |
...
|---|---|---|---|
| 1 | `SW-W2-S1` | `cursor/v23-w2-s1-scheduler-service` | required |
| 2 | `SW-W2-S2` | `cursor/v20-w2-s2-native-loop` | required |

## 2. Setup
```bash
git status --porcelain            # must print nothing; otherwise STOP (M1)
git fetch origin
git checkout -B cursor/v23-merge-w2 origin/cursor/sw-v23-integration-460c
...
sudo -u postgres psql -c "CREATE DATABASE swarm_sw_merge_w2 OWNER swarm;" || true
export SWARM_DATABASE_URL=postgresql+psycopg://swarm:swarm@127.0.0.1:5432/swarm_sw_merge_w2
...
## 5. Handoff and push
Write `docs/v2.3/sessions/SW-MERGE-W2.md` with: BASE, the new head, one line per session (merged / already merged / not pushed / not ready / conflict / fails verify), every section 4 result line, and the section 8 packet. Then:
```bash
git add docs/v2.3/sessions/SW-MERGE-W2.md && git commit -m "docs(v2.3): SW-MERGE-W2 handoff"
...
```markdown
### Codex review packet — SW-MERGE-W2
